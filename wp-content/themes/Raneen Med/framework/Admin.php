...
class ThematerAdmin
{
    ...
    function loadMenu()
	{
	   add_theme_page(" خيارات القالب","خيارات قالب تو ايجى", 'administrator', 'themater',  array(&$this, 'ThematerThemeOptions'));
	}
    ...
    function loadHead()
	{
		...
		echo "<script type='text/javascript' src='" . THEMATER_ADMIN_URL . "/js/custom.js'></script> \n";
		echo "<script type='text/javascript' src='" . THEMATER_ADMIN_URL . "/js/jquery.easing-1.3.pack.js'></script> \n";
    ...
    function ajax_savechanges()
    {
        ...
        echo '<span style="color:green; font-weight:bold;">تم حفظ التغييرات!</span>';
    }
    ...
    function ajax_admin_options()
    {
        ...
        if($do == 'reset') {
            ...
            echo '<div class="tt-success">تمت العودة الى قيم القالب الافتراضية. <a href="'. admin_url() . 'themes.php?page=themater">اضغط هنا</a> لاعادة تحميل صفحة اعدادات القالب.</div>';
        }
    }
    ...
    {
        $skip = array('type', 'name', 'value', 'help', 'priority', 'display', 'options', 'sep', 'prefix', 'suffix', 'callback');
        $returnval = '';
	...
	function form_wrapTop ($name, $attributes = array())
	{
        $prefix = isset($attributes['prefix']) ? $attributes['prefix'] : '';
        $suffix = isset($attributes['suffix']) ? $attributes['suffix'] : '';
		return $prefix . '<div class="wrapTop"></div>' . $suffix;
	}
	
	function form_wrapEnd ($name, $attributes = array())
	{
        $prefix = isset($attributes['prefix']) ? $attributes['prefix'] : '';
        $suffix = isset($attributes['suffix']) ? $attributes['suffix'] : '';
		return $prefix . '<div class="wrapEnd">' . $suffix;
	}
	
	function form_sep ($name, $attributes = array())
	{
        $prefix = isset($attributes['prefix']) ? $attributes['prefix'] : '';
        $suffix = isset($attributes['suffix']) ? $attributes['suffix'] : '';
		return $prefix . '<div class="sep"></div>' . $suffix;
	}
    ...
    function form_imageupload ($name, $attributes = array())
    {
        ...
        <div style="padding-bottom:10px;">
            <a id="themater_image_upload_<?php echo $name; ?>" class="tt_imageupload button" > ارفع الان</a>
            <a <?php if(!$current_image) { ?> style="display: none;" <?php }?> id="<?php echo $name; ?>_reset" title="<?php echo $name; ?>" class="button tt_imageupload_reset" >ازالة</a>
        </div>
    ...
    ?>
        ...
        <form id="ttForm" method="POST">
            ...
            <h2>خيارات القالب| 
             ...
             <div class="tt-wrap">
             <div class="nasjHeader">
             <div class="shb"><a href="http://www.nasj.com/" target="new">&nbsp;</a></div>
              <div class="shb_sup"><a href="http://ms.nasj.com/" target="new">&nbsp;</a></div>
             	<div class="tt-header" > <?php $this->optionsPageMenu(); ?></div>
             </div>
...
<!--                     <div class="tt-sidebar">
                    ...
                    <?php 
                            if($optionsPageContent != 'no-options') {
                            ?>   
                            <div style="clear: both; margin: 15px 0 0 0; padding: 15px;text-align: center; background: #f0f0f0; border-top: 1px solid #C6C6C6 ;">
                                <a class="button-primary" onclick="themater_savechanges('savechanges','ttForm','ttSaveChanges');">حفظ التتغيرات</a>
    ...
    {
        $valid_form_elements = array('text', 'textarea', 'checkbox', 'checkboxes', 'radio', 'select', 'hidden', 'content', 'wrapEnd', 'wrapTop', 
		'callback', 'colorpicker', 'colorpicker2', 'imageupload', 'fileupload', 'raw');       
        ?>
            <?php
                        ...
                        ?>
                                <div class="tt-form-element">
                                    <?php if($item_display == 'inline') {
										...
										/*****/
										} elseif($item_display == 'wrapTop') { ?>
                                        <div class="myWrap">
                                        <?php
										
										} elseif($item_display == 'wrapEnd') { ?>
                                        </div>
                                        <?php
										
										
										} elseif($item_display == 'sep') { ?>
                                        <div class="sep"></div>
                                        <?php
