...
class ThematerBanners125 extends WP_Widget 
{
    ...
    {
        $widget_options = array('description' => __('اضافة مساحة اعلانية 250 * 300.', 'themater') );
        $control_options = array( 'width' => 600);
		$this->WP_Widget('themater_banners_125', '&raquo;شبكات-  مساحة اعلانية', $widget_options,$control_options);
    }
    ...
    function widget($args, $instance)
    {
        ...
        $returnval = '';
         if(is_array($get_banners)) {
            $returnval .= '<div class="google_ads"><ul class="widget-container"><li class="banners-125">';
    ...
    {	
        ...
        <div style="margin-bottom: 20px;">
            <a class="button" onclick="themater_125_banner_new();" >اضافة اعلان جديد</a> &nbsp; &nbsp; <input type="checkbox" name="<?php echo $this->get_field_name('randomize'); ?>" <?php checked('true', $instance['randomize']); ?> value="true" /> اظهار عشوائي للاعلانات
        </div>
        <?php
            if(is_array($get_banners)) {
                foreach($get_banners as $banner_id=>$banner_source) {
                    ...
                    <div class="tt-clearfix" style="padding: 0 0 20px 0; border-bottom: 1px solid #ddd; margin-bottom: 20px;" id="container_<?php echo $this->get_field_id($banner_id); ?>">
                        <div style="width: 133px; float: right; ">
                            ...
                            <div><a class="button"  onclick="themater_125_banner_preview('<?php echo $this->get_field_id($banner_id); ?>');">عرض</a> <a class="button tt-button-red" onclick="if (confirm('The selected banner will be deleted! Do you really want to continue?')) { themater_125_banner_delete('<?php echo $this->get_field_id($banner_id); ?>'); } return false;">حذف</a></div>
                        </div>
                ...
                <div class="themater_125_banner_prototype" style="display: none;">
                    <div class="tt-clearfix" style="padding: 0 0 20px 0; border-bottom: 1px solid #ddd; margin-bottom: 20px;" id="container_the__id__">
                        <div style="width: 133px; float: right; ">
                            ...
                            <div><a class="button"  onclick="themater_125_banner_preview('the__id__');">عرض</a> <a class="button tt-button-red" onclick="if (confirm('هل تريد حذف الاعلان ؟')) { themater_125_banner_delete('the__id__'); } return false;">حذف</a></div>
                        </div>
                        <div style="margin-right: 150px;">
